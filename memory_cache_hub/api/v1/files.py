...
from memory_cache_hub.api.v1.types import FileUpload
import os
import shutil

router = APIRouter()

@router.post("/upload/", status_code=200, tags=["files"])
...
@router.delete("/delete/", status_code=200, tags=["files"])
...
@router.get("/list/{project_name}/", response_model=List[str], tags=["files"])
